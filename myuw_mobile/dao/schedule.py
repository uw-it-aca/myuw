...
from myuw_mobile.dao.term import get_current_summer_term, get_next_autumn_quarter
...
def filter_schedule_sections_by_summer_term(schedule, summer_term):
    ...
    """
    if has_summer_quarter_section(schedule) and is_half_summer_term(summer_term):
        filtered_sections = []
        for section in schedule.sections:
            if is_full_summer_term(section.summer_term) or is_same_summer_term(section.summer_term, summer_term):
...
def has_summer_quarter_section(schedule):
    ...
    """
    return schedule is not None and len(schedule.sections) > 0 and schedule.term.quarter == "summer"
