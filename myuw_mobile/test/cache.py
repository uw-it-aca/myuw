...
from myuw_mobile.util.cache_implementation import MyUWCache
...
class TestCustomCachePolicy(TestCase):
    def test_sws_default_policies(self):
        with self.settings(RESTCLIENTS_DAO_CACHE_CLASS='myuw_mobile.util.cache_implementation.MyUWCache'):
            cache = MyUWCache()
            ...
            cache.processResponse("sws", "/student/myuwcachetest1", ok_response)
            response = cache.getCache('sws', '/student/myuwcachetest1', {})
            ...
            cache_entry.time_saved = orig_time_saved - timedelta(minutes=(60 * 4)-2)
            cache_entry.save()
            ...
            cache_entry.time_saved = orig_time_saved - timedelta(minutes=(60 * 4)+1)
            cache_entry.save()
    ...
    def test_sws_term_policy(self):
        with self.settings(RESTCLIENTS_DAO_CACHE_CLASS='myuw_mobile.util.cache_implementation.MyUWCache'):
            ...
            response = cache.getCache('sws', '/student/v5/term/1014,summer.json', {})
            self.assertEquals(response, None)
            cache.processResponse("sws", "/student/v5/term/1014,summer.json", ok_response)
            response = cache.getCache('sws', '/student/v5/term/1014,summer.json', {})
            self.assertEquals(response["response"].data, 'xx')

            cache_entry = CacheEntryTimed.objects.get(service="sws",
                                                      url="/student/v5/term/1014,summer.json")
    ...
    def test_default_policies(self):
        with self.settings(RESTCLIENTS_DAO_CACHE_CLASS='myuw_mobile.util.cache_implementation.MyUWCache'):
            ...
            self.assertEquals(response, None)
            cache.processResponse("no_such", "/student/myuwcachetest1", ok_response)
            response = cache.getCache('no_such', '/student/myuwcachetest1', {})
            self.assertEquals(response["response"].data, 'xx')

            cache_entry = CacheEntryTimed.objects.get(service="no_such",
                                                      url="/student/myuwcachetest1")
            # Cached response is returned after 3 hours and 58 minutes
            orig_time_saved = cache_entry.time_saved
            cache_entry.time_saved = orig_time_saved - timedelta(minutes=(60 * 4)-2)
