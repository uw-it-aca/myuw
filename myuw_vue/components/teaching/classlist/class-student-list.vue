    ...
    >
      <template #card-heading>
        <div v-if="sectionData" class="d-flex flex-wrap justify-content-between">
          ...
          <span v-if="sectionData.sln"> SLN {{ sectionData.sln }} </span>
        </div>
      </template>
      ...
      <template #card-body>
        <div class="alert alert-light border myuw-text-md">
          ...
          <a href="https://wellbeing.uw.edu/resources/pronoun-usage-expectations">Expectations and Guidance on Student Pronouns</a>. 
      ...
      <template v-if="noData" #card-error> No class information was found. </template>
      ...
      <template v-else-if="dataError" #card-error>
        An error occurred and MyUW cannot load the class student information right now. Please try
        again later.
      </template>
    </uw-card>

    <uw-course-stats :loaded="isReady" :section-data="sectionData ? sectionData : {}" />
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    showContent() {
      return (
        this.sectionData &&
        this.sectionData.sections.length &&
        this.sectionData.sections[0].registrations.length
      );
    },
    showCard() {
      return this.instructor && (!this.isReady || this.isErrored || this.showContent);
    },
    getErrorCode() {
    ...
    dataError() {
      return this.isErrored && !(this.noData || this.noAccessPermission || this.invalidCourse);
    },
    jointSections() {
