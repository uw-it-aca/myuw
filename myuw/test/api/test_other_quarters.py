...
class TestOtherQuarters(MyuwApiTest):
    ...
    def test_javerage_oquarters(self):
        ...
        response = self.get_oquarters_response()
        data = json.loads(response.content)
        self.assertEqual(data["next_term_data"]["has_registration"], True)
        ...
        self.assertEqual(data["terms"][0]['summer_term'], 'a-term')
        self.assertEqual(data["terms"][0]['year'], 2013)
        self.assertEqual(data["terms"][0]['quarter'], 'Summer')
        self.assertEqual(data["terms"][0]['credits'], '2.0')
        self.assertEqual(data["terms"][0]['section_count'], 2)
        self.assertEqual(data["terms"][0]['url'], '/2013,summer,a-term')
