...
class TestInstructorSchedule(MyuwApiTest):
    ...
    def test_bill_current_term(self):
        ...
        response = self.get_instructor_schedule_res('bill')
    ...
    def test_none_current_term(self):
        ...
        response = self.get_instructor_schedule_res('none')
        self.assertEquals(response.status_code, 404)
        self.assertEquals(response.content, 'Data not found')

    def test_eight_current_term(self):

        response = self.get_instructor_schedule_res('eight')
        self.assertEquals(response.status_code, 200)
    ...
    def test_jbothell_current_term(self):
        ...
        response = self.get_instructor_schedule_res('jbothell')
        self.assertEquals(response.status_code, 200)
    ...
    def test_missing_current_term(self):
        ...
        response = self.get_instructor_schedule_res('jerror')
        self.assertEquals(response.status_code, 543)

    def test_summer_terms(self):

        response = self.get_instructor_schedule_res('javerage', '2013-07-06')
        self.assertEquals(response.status_code, 200)
        data = json.loads(response.content)
        ...
        response = self.get_instructor_schedule_res('javerage', '2013-07-25')
        data = json.loads(response.content)
        self.assertEquals(data["summer_term"], "b-term")

    def get_instructor_schedule_res(self, user=None, date=None):
        ...
        return self.get_response_by_reverse('myuw_instructor_schedule')
