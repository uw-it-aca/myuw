...
from myuw.dao.card_display_dates import (
    ...
    get_card_visibilty_date_values,
    get_reg_data,
    in_show_grades_period,
    is_before_bof_term,
    is_before_eof_7d_after_class_start,
    is_before_eof_finals_week,
    is_before_last_day_of_classes,
)
from myuw.dao.term import get_comparison_datetime, get_specific_term
from myuw.test import fdao_sws_override, get_request, get_request_with_date
...
class TestDisplayValues(TestCase):
    ...
    def test_is_before_eof_7days_of_term(self):
        ...
        now_request = get_request_with_date('2013-04-08')
        values = get_card_visibilty_date_values(now_request)
        self.assertTrue(values["is_before_eof_7days_of_term"])
        now = get_comparison_datetime(now_request)
        self.assertTrue(
        ...
        now_request = get_request_with_date('2013-04-09')
        values = get_card_visibilty_date_values(now_request)
        self.assertFalse(values["is_before_eof_7days_of_term"])
        now = get_comparison_datetime(now_request)
        self.assertFalse(
        ...
        now_request = get_request_with_date('2012-08-21')
        values = get_card_visibilty_date_values(now_request)
        self.assertFalse(values["is_after_grade_submission_deadline"])
        ...
        now_request = get_request_with_date('2012-10-01')
        values = get_card_visibilty_date_values(now_request)
        self.assertTrue(values["is_before_eof_7days_of_term"])
        now = get_comparison_datetime(now_request)
        self.assertTrue(
        ...
        now_request = get_request_with_date('2012-10-02')
        values = get_card_visibilty_date_values(now_request)
        self.assertFalse(values["is_before_eof_7days_of_term"])
        now = get_comparison_datetime(now_request)
        self.assertFalse(
        ...
        now_request = get_request_with_date('2013-01-14')
        values = get_card_visibilty_date_values(now_request)
        self.assertTrue(values["is_before_eof_7days_of_term"])
        now = get_comparison_datetime(now_request)
        self.assertTrue(
        ...
        now_request = get_request_with_date('2012-06-25')
        values = get_card_visibilty_date_values(now_request)
        self.assertTrue(values["is_before_eof_7days_of_term"])
        now = get_comparison_datetime(now_request)
        self.assertTrue(
        ...
        # A-term, Full-term
        now_request = get_request_with_date('2012-06-26')
        values = get_card_visibilty_date_values(now_request)
        self.assertFalse(values["is_before_eof_7days_of_term"])
        now = get_comparison_datetime(now_request)
        self.assertFalse(
        ...
        # B-term
        now_request = get_request_with_date('2012-07-26')
        values = get_card_visibilty_date_values(now_request)
        now = get_comparison_datetime(now_request)
        self.assertFalse(
            is_before_eof_7d_after_class_start(now, now_request))
    ...
    def test_last_final_exam_day(self):
        # spring
        now_request = get_request_with_date('2013-06-14')
        values = get_card_visibilty_date_values(now_request)
        self.assertTrue(values["is_after_last_day_of_classes"])
        self.assertTrue(values["is_before_end_of_finals_week"])
        now = get_comparison_datetime(now_request)
        ...
        now_request = get_request_with_date('2013-06-15')
        values = get_card_visibilty_date_values(now_request)
        ...
        now_request = get_request_with_date('2013-12-14')
        values = get_card_visibilty_date_values(now_request)
        self.assertFalse(values["is_before_end_of_finals_week"])
        now = get_comparison_datetime(now_request)
        self.assertFalse(is_before_eof_finals_week(now, now_request))
        ...
        self.assertTrue(values["is_before_end_of_finals_week"])
        now_request = get_request_with_date('2013-03-23')
        values = get_card_visibilty_date_values(now_request)
        self.assertFalse(values["is_before_end_of_finals_week"])
        now = get_comparison_datetime(now_request)
        self.assertFalse(is_before_eof_finals_week(now, now_request))
    ...
    def test_myplan_peak_loads(self):
        ...
        for date in peak_load_false:
            ...
            self.assertFalse(
                ...
                f'Expected MyPlan peak load to be False on date {date}')

        for date in peak_load_true:
            values = self.get_visibility_for_date(date)
            self.assertTrue(
                values['myplan_peak_load'],
                f'Expected MyPlan peak load to be True on date {date}')
