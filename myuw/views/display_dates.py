...
from myuw.dao.term import get_comparison_datetime, get_default_datetime
...
def add_date_term_info(request, context):
    ...
    try:
        ...
        logger.error(f"get_term_by_date: {ex}")
    ...
    for val in DATE_KEYS:
        if val in request.session:
            if request.session[val] is True:
                context[f"{val}_true"] = True
            else:
                context[f"{val}_false"] = True
        else:
            context[f"{val}_unset"] = True
