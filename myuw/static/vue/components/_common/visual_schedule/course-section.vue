<template>
  <div class="course-section flex-fill" :style="computedStyles">
    <div role="group" tabindex="0" class="course-section-inner" :aria-label="ariaLabel">
      <div :class="`bg-c${meetingData.section.color_id}`" class="p-1 text-center myuw-text-xxs">
        ...
        <a :href="sectionUrl" class="text-white" :aria-label="`Course section: ${sectionTitle}`">
          {{ sectionTitle }}
        </a>
      </div>
      ...
      <div class="p-1 text-center myuw-text-xxs">
        <slot>
          ...
          <a v-if="showConfirmLink" :href="confirmationLink" target="_blank" class="d-block">
            (Confirm)
          </a>
        </slot>
      </div>
...
export default {
  ...
  computed: {
    ...
    sectionUrl: function () {
      return `/academics/#${this.meetingData.section.curriculum_abbr}-${this.meetingData.section.course_number}-${this.meetingData.section.section_id}`;
    },
