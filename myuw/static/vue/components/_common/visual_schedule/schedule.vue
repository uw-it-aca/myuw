  ...
  >
    ...
    <template #card-body>
      ...
      >
        ...
        >
          <!-- tab content -->
          <uw-schedule-tab :period="period" :term="allSchedules[termLabel].term" />
        </b-tab>
      </b-tabs>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters('visual_schedule', ['isReadyTagged', 'isErroredTagged']),
    isReady() {
    ...
    termName: function () {
      ...
      if (termData.summer_term) {
        name += ' ' + termData.summer_term.split('-').map(this.ucfirst).join('-');
      }
      return name;
    },
