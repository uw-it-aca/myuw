...
const fetchBuilder = (url, postProcess, type) => {
  return ({ commit, getters }, urlExtra = '') => {
    if (!getters.isReadyTagged(urlExtra) && !getters.isFetchingTagged(urlExtra)) {
      ...
      axios
        ...
        .then(({ data, statusCode }) => {
          ...
          commit('setStatus', { [urlExtra]: { type: statusOptions[0], code: statusCode } });
        })
        .catch(error => {
...
const buildWith = ({
  ...
  const getters = {
    ...
    isReady(state) {
      return state.status[''] !== undefined && state.status[''].type == statusOptions[0];
    },
    isFetching(state) {
      return state.status[''] !== undefined && state.status[''].type == statusOptions[1];
    },
    isErrored(state) {
      return state.status[''] !== undefined && state.status[''].type == statusOptions[2];
    },
    statusCode(state) {
    ...
    isReadyTagged: state => urlExtra =>
      state.status[urlExtra] !== undefined && state.status[urlExtra].type == statusOptions[0],
    isFetchingTagged: state => urlExtra =>
      state.status[urlExtra] !== undefined && state.status[urlExtra].type == statusOptions[1],
    isErroredTagged: state => urlExtra =>
      state.status[urlExtra] !== undefined && state.status[urlExtra].type == statusOptions[2],
