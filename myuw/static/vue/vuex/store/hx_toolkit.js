...
const postProccess = (response, urlExtra) => {
  ...
  if (htmlDoc.getElementsByClassName('myuw-card-image-full')[0]) {
    ...
    proccessValue[urlExtra].src = htmlDoc.getElementsByClassName('myuw-card-image-full')[0].src;
    proccessValue[urlExtra].alt = htmlDoc.getElementsByClassName('myuw-card-image-full')[0].alt;
  }

  if (htmlDoc.getElementsByClassName('myuw-highlight')[0]) {
    proccessValue[urlExtra].articleTeaserTitle = htmlDoc.getElementsByClassName(
      'myuw-highlight'
    )[0].innerHTML;
  }

  if (htmlDoc.getElementsByClassName('myuw-highlight')[1]) {
    proccessValue[urlExtra].articleTeaserBody = htmlDoc.getElementsByClassName(
      'myuw-highlight'
    )[1].textContent;
    if (htmlDoc.getElementsByClassName('myuw-highlight')[1].children[0]) {
      proccessValue[urlExtra].articleFaClass = htmlDoc
        .getElementsByClassName('myuw-highlight')[1]
        .children[0].classList[1].slice(3);
    }
  }
