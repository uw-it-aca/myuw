...
describe('Grad Status Card', () => {
  ...
  it('Check status changes on fetch', async () => {
    ...
    expect(grad.getters.isFetching(wrapper.vm.$store.state.grad)).toBeTruthy();
    // It takes like 10 ms to process the mock data through fetch postProcess
    await new Promise(r => setTimeout(r, 10));
    expect(grad.getters.isReady(wrapper.vm.$store.state.grad)).toBeTruthy();
    expect(grad.getters.isErrored(wrapper.vm.$store.state.grad)).toBeFalsy();
    expect(grad.getters.statusCode(wrapper.vm.$store.state.grad)).toEqual(200);
  });
