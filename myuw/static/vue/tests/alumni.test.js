...
describe('Alumni Card', () => {
  ...
  it('Display card if user is alumni', () => {
    ...
    expect(link1.attributes().href).toBe('https://www.washington.edu/alumni');

    let link2 = wrapper.findAll('a').at(1);
    expect(link2.text()).toBe('Columns Magazine');
    expect(link2.attributes().href).toBe('https://magazine.washington.edu/');

    let link3 = wrapper.findAll('a').at(2);
    expect(link3.text()).toBe('Learn about the UWAA');
    expect(link3.attributes().href).toBe('https://www.washington.edu/cms/alumni/membership/');
  });
