...
describe('Adviser store', () => {
  ...
  it('Check status changes on fetch - success', () => {
    ...
    return expectAction(advisers.actions.fetch, null, advisers.state, getters, [
  ...
  it('Check postProcess - javerage', async () => {
    ...
    expect(data[0].program).toBe("OMAD Advising");
    expect(data[0].program).toBe("UW Honors");
    expect(data[0].program).toBe("Robinson Center");
    expect(data[0].program).toBe("Athletics – SAAS");
  });

  it('Check postProcess - inactive', async () => {
