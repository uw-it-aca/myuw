...
describe('Notice Card', () => {
    ...
    it('Check status changes on fetch', async () => {
        ...
        expect(notices.getters.isFetching(wrapper.vm.$store.state.notices)).toBeTruthy();
    ...
    it('Check status when fetch fails', async () => {
        ...
        // It takes like 10 ms to process the mock data through fetch postProcess
        await new Promise((r) => setTimeout(r, 10));
        expect(notices.getters.isErrored(wrapper.vm.$store.state.notices)).toBeTruthy();
        expect(notices.getters.isReady(wrapper.vm.$store.state.notices)).toBeFalsy();
    });
    ...
    it('Check postProcess fields', async () => {
        ...
        wrapper.vm.$store.state.notices.value.forEach(notice => {
            if ("notice_title" in notice) {
                const htmlDoc = parser.parseFromString(notice.notice_title, 'text/html');
                expect(htmlDoc.getElementsByClassName('notice-title')).toHaveLength(1);
            }
            if ("notice_body" in notice) {
                const htmlDoc = parser.parseFromString(notice.notice_body, 'text/html');
                expect(htmlDoc.getElementsByClassName('notice-body-with-title')).toHaveLength(1);
            }
            if ("date" in notice) {
