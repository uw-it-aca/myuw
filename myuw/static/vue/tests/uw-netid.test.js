...
describe('UWNetID Card', () => {
  ...
  it('Check status changes on fetch', async () => {
    ...
    expect(profile.getters.isFetching(wrapper.vm.$store.state.profile)).toBeTruthy();
    // It takes like 10 ms to process the mock data through fetch postProcess
    await new Promise(r => setTimeout(r, 10));
    expect(profile.getters.isReady(wrapper.vm.$store.state.profile)).toBeTruthy();
    expect(profile.getters.isErrored(wrapper.vm.$store.state.profile)).toBeFalsy();
  });
  ...
  it('Check status when fetch fails', async () => {
    ...
    // It takes like 10 ms to process the mock data through fetch postProcess
    await new Promise(r => setTimeout(r, 10));
    expect(profile.getters.isErrored(wrapper.vm.$store.state.profile)).toBeTruthy();
    expect(profile.getters.isReady(wrapper.vm.$store.state.profile)).toBeFalsy();
  });
