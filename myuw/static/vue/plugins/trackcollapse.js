...
// Collapsable button selector
const CB_SELECTOR = 'button.collapsed, button.not-collapsed';
  ...
  Vue.directive('no-track-collapse', {
    bind: (el, _binding, _vnode) => {
      const elm = el.tagName === 'BUTTON' ? el :
        el.querySelector(`${CB_SELECTOR}:not(.no-track-collapse)`);
    ...
    updated() {
      if (this.$el && this.$el.querySelectorAll) {
        this.$el.querySelectorAll(
          `${CB_SELECTOR}:not(.no-track-collapse):not(.track-collapse)`
        ).forEach((el) => {
